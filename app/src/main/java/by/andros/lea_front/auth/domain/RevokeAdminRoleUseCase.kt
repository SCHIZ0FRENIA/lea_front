...
class RevokeAdminRoleUseCase @Inject constructor(
    ...
    suspend operator fun invoke(userId: String): Result<UserRoleResponse> {
        ...
        if (!repository.isUserAdmin()) {
            return Result.failure(SecurityException("Only administrators can revoke admin privileges"))
        }
        
        return repository.revokeAdminRole(userId)
    }
} 